...
def create_latent_walk_for_cnn(
    samples, filename="walk.mp4", fps=60, filters_per_row=8, filter_upscale=80
):
    """
    ...
    filters = samples[..., :num_weights].reshape(
        n_samples * n_steps * n_videos * outc, *conv2d_shape[1:]
    )
    ...
    for t in range(n_frames):
        input = filters[t * n_videos : (t + 1) * n_videos].view(
            -1, inc, kh * filter_upscale, kw * filter_upscale
        )
        frame = make_grid(
            input,
            normalize=True,
            scale_each=True,
            nrow=filters_per_row,
            padding=2 * filter_upscale,
            pad_value=0,
        )
        ndarr = (
            frame.mul(255)
