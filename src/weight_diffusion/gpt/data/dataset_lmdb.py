...
from weight_diffusion.gpt.data.database import Database
from weight_diffusion.gpt.data.augment import random_permute_flat
from weight_diffusion.gpt.data.normalization import get_normalizer
from weight_diffusion.gpt.tasks import TASK_METADATA
from weight_diffusion.gpt.download import find_checkpoints
...
class ParameterDataset(Dataset):
    ...
    target_epoch_size: int = (
        806400  # Amount of data to train on per-"epoch" (806400 is arbitrary)
    )
    ...
    max_step_spacing: int = (
        None  # Maximum spacing between starting and future checkpoints
    )
    normalizer_name: str = "openai"  # Parameter normalization algorithm
    openai_coeff: float = (
        4.185  # Scaling coefficient for the "openai" DALL-E 2 normalizer
    )
    single_run_debug: bool = False  # Option to debug with a single run
    min_val: float = (
        None  # Minimum value of parameters (usually passed to test dataset)
    )
    max_val: float = (
        None  # Maximum value of parameters (usually passed to test dataset)
    )
    permute_augment: bool = (
        False  # if True, applies permutation augmentation to parameters
    )
    ...
    def __post_init__(self):
        ...
        if self.verify_done:
            lmdb_paths = [
                p for p in lmdb_paths if os.path.exists(os.path.join(p, "DONE.txt"))
            ]
        ...
        if self.split == "train":
            ...
            self.runs = (
                self.runs[-self.num_test_runs :] if self.num_test_runs > 0 else []
            )
        ...
        self.databases = {
            lmdb_path: Database(path=lmdb_path, map_size=10485760)
            for lmdb_path in lmdb_paths
        }
        ...
        self.parameter_sizes = (
            self.get_database(0)[
                f'{list(self.run_jsons[0]["checkpoints"].keys())[0]}_arch'
            ]
            .long()
            .tolist()
        )
        ...
        self.num_checkpoints = [
            len(run_json["checkpoints"]) for run_json in self.run_jsons
        ]
        self.generator = None
    ...
    def __getitem__(self, run_index: int) -> Dict[str, Any]:
        ...
        max_step_spacing = (
            (num_checkpoints - 1)
            if self.max_step_spacing is None
            else self.max_step_spacing
        )
        ...
        parameters_0, parameters_1 = self.aug_fn(
            (parameters_0, parameters_1), seed=None
        )
