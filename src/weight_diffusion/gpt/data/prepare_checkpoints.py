...
from weight_diffusion.gpt.data.dataset_lmdb import ParameterDataset
from weight_diffusion.gpt.diffusion import create_diffusion
import shutil
...
if __name__ == "__main__":
    ...
    if compute_variance:
        ...
        rand_iters = torch.randint(
            low=0, high=num_checkpoints_per_run, size=(num,)
        ).tolist()
        runs_and_iters = zip(rand_runs, rand_iters)
        nets = [
            dataset.get_run_network(run, iteration)
            for run, iteration in tqdm(runs_and_iters, total=num)
        ]
