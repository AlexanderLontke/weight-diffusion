...
import torch
import os
from weight_diffusion.gpt.distributed import is_main_proc, synchronize
...
def find_model(model_name):
    ...
    if (
        ...
        assert os.path.isfile(
            model_name
        ), f"Could not find G.pt checkpoint at {model_name}"
...
def download_model(model_name):
    ...
    if (
        not os.path.isfile(local_path) and is_main_proc()
    ):  # download (only on primary process)
        os.makedirs("pretrained_models", exist_ok=True)
        web_path = (
            f"http://learn2learn.eecs.berkeley.edu/pretrained_models/{model_name}"
        )
...
def find_checkpoints(dataset_path):
    ...
    if dataset_name in checkpoint_datasets:
        ...
        assert os.path.isdir(
            dataset_path
        ), f"Could not find a checkpoint dataset at {dataset_path}"
        return dataset_path
